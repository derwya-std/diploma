...
class IMUNormalizer(nn.Module):
    def __init__(self, imu_dim: int = 6):
        super().__init__()
        self.register_buffer("imu_mean", torch.zeros(imu_dim))
        self.register_buffer("imu_std", torch.ones(imu_dim))

    def forward(self, x: torch.Tensor):
        return (x - self.imu_mean) / (self.imu_std + 1e-6)


# ───────────────────────────
#  CBAM residual blocks
# ───────────────────────────
class CBAM(nn.Module):
    def __init__(self, c, r=16):
        super().__init__()
        self.mlp = nn.Sequential(nn.Linear(c, c // r, False), nn.ReLU(True),
                                 nn.Linear(c // r, c, False))
    ...
    def forward(self, x):
        ...
        ca = torch.sigmoid(
            self.mlp(x.mean((2, 3))) +
            self.mlp(x.view(b, c, -1).max(-1)[0])
        ).view(b, c, 1, 1)
        x = x * ca
        sa = torch.sigmoid(self.spconv(
            torch.cat([x.mean(1, True), x.max(1, True)[0]], 1)))
        return x * sa
...
class ResCBAM(nn.Module):
    def __init__(self, cin, cout, stride=1, dil=1):
        ...
        self.down = nn.Identity() if (stride == 1 and cin == cout) else nn.Sequential(
...
class IMUAttention(nn.Module):
    ...
    def forward(self, v, imu):  # v:(BT,d) imu:(BT,M,d)
        a = self.e(torch.tanh(self.q(v).unsqueeze(1) + self.k(imu)))
        w = torch.softmax(a, 1)
        return (w * imu).sum(1)  # (BT,d)
...
class VIOCNN(nn.Module):
    def __init__(
            ...
            img_channels: int = 3,
            imu_dim: int = 6,
            emb_dim: int = 192,  # unified embedding
            hidden_size: int = 256,
            gru_layers: int = 3,
        ...
        self.gru = nn.GRU(emb_dim, hidden_size, gru_layers,
                          batch_first=True, bidirectional=True,
                          dropout=0.25 if gru_layers > 1 else 0.0)

        self.pose_fc = nn.Linear(hidden_size, 6)
        self.conf_fc = nn.Linear(hidden_size, 1)

    # ───────────
    def forward(self, images: torch.Tensor, imu: torch.Tensor,
                seq_len: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        B, T, C, H, W = images.shape
        _, _, M, _ = imu.shape

        v_feat = self.frame_enc(images.view(B * T, C, H, W))
        imu_emb = self.imu_fc(self.imu_norm(imu.view(B * T * M, -1)))
        imu_emb = self.imu_ln(imu_emb).view(B * T, M, -1)

        fused = self.attn(v_feat, imu_emb).view(B, T, -1)
        fused = self.drop(fused)

        packed, order = nn.utils.rnn.pack_padded_sequence(
            fused, seq_len.cpu(), batch_first=True, enforce_sorted=False), None
        ...
        poses = self.pose_fc(out)  # (B,T,6)
        conf = torch.sigmoid(self.conf_fc(out))  # (B,T,1)
        return poses, conf
